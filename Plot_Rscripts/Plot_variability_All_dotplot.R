...
Plot_variability_All_dotplot <- function() {
  par(mfrow=c(2,2))    # set the plotting area into a 1*2 array
  require(gridExtra)
  plot1 <- ggplot(mydata, aes(x=Genotype, y=((EdU_blank+Blank_blank)/bTotal), fill=Animal)) +
  ...
  plot2 <- ggplot(mydata, aes(x=Genotype, y=((EdU_BrdU+Blank_BrdU)/bTotal), fill=Animal)) +
    geom_dotplot(binaxis='y', stackdir='center', position=position_dodge(0.4), color="red") +
    ylim(0,1)
  plot2 <- plot2 + stat_summary(fun.data=mean_sdl, fun.args = list(mult=1), 
                                geom="pointrange", color="black", size=0.1, position=position_dodge(0.4))
  
  plot3 <- ggplot(mydata, aes(x=Genotype, y=((EdU_pH3G2+Blank_pH3G2)/bTotal), fill=Animal)) +
    geom_dotplot(binaxis='y', stackdir='center', position=position_dodge(0.4), color="red") +
    ylim(0,1)
  plot3 <- plot3 + stat_summary(fun.data=mean_sdl, fun.args = list(mult=1), 
                                geom="pointrange", color="black", size=0.1, position=position_dodge(0.4))
  
  plot4 <- ggplot(mydata, aes(x=Genotype, y=((EdU_pH3M+Blank_pH3M)/bTotal), fill=Animal)) +
    geom_dotplot(binaxis='y', stackdir='center', position=position_dodge(0.4), color="red") +
    ylim(0,1)
